...
with tempfile.TemporaryDirectory(prefix='rx-operator-delivery-') as temporary:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            page.get_by_label('Account',exact=True).fill(browser_data['principal'])
            page.get_by_label('Password',exact=True).fill(browser_data['password'])
            page.get_by_role('button',name='Sign in',exact=True).click()
            expect(page.get_by_role('button',name='Sign out',exact=True)).to_be_visible()
            page.evaluate('document.fonts.ready')
            assert page.evaluate('(font) => document.fonts.check(font, "Operating condition check")', '400 16px "IBM Plex Sans KR"')
