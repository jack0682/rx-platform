...
def run(c:dict)->None:
    ...
    observations={
        ...
        'EQUIPMENT':{'assertions':{'file_device_current_sources':bool(equipment['sources']) and all(v['usable'] for v in equipment['sources']),'host_is_real_product_process':host['phase']=='SOFTWARE_READY_UNARMED','same_kernel_clock':host['clock_id']==c['installation']['clock_id'],'unchanged_durable_host_journals':current_journals==original_journals},'observations':{'host':host,'diagnostics':equipment,'installation_journals':current_journals},'limitations':['FILE_SIMULATION health is not a physical interlock or mechanical qualification.']},
